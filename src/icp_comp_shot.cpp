...
IcpMapping::IcpMapping(){
    ...
    trans = get_transfrom(lidarFrame);
}
...
void IcpMapping::pcCallback(const sensor_msgs::PointCloud2ConstPtr &pc){
    ...
    crop.setMax(Eigen::Vector4f(30,15,1.8,1.0));  //数据随意给的，具体情况分析
    ...
    toROSMsg(*cropped_map, ros_cloud_msg);
	ros_cloud_msg.header.frame_id = baseFrame;
    map_filtered_pub.publish(ros_cloud_msg);
    ...
    crop.setMin(Eigen::Vector4f(-30,-30,1.5,1.0)); //给定立体空间
    crop.setMax(Eigen::Vector4f(30,30,1.8,1.0));  //数据随意给的，具体情况分析
    ...
    crop.setMin(Eigen::Vector4f(-5,-2,-10,1.0)); //给定立体空间
    crop.setMax(Eigen::Vector4f(5,2,30,1.0));  //数据随意给的，具体情况分析
    crop.setInputCloud(pc_input);
    crop.setKeepOrganized(true);
    crop.setUserFilterValue(0.05f);
